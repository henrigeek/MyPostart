...
from postart.models import Tag, Image, Album, Author
...
def logout_view(request):
    ...
    return HttpResponseRedirect("/postart/")
...
class AuthorCreate(CreateView):
    def get_success_url(self):
        return reverse('postart:create-user',args=(self.object.pk,))
    model = Author
...
class ImageCreate(CreateView):
    ...
    def get_success_url(self,*args,**kwargs):
        return reverse('postart:image-view', kwargs={'pk' : self.object.pk})
...
def ImageLike(request, pk):
    if pk:
        ...
        a.rating=count
        a.save()
    return HttpResponseRedirect('/postart/image/%s' %pk)
...
def CreateUser(request, pk):
    if pk:
        ...
        user.save()
    return HttpResponseRedirect('/postart/author/%s' %pk)
